...
def parse_args():
    ...
    parser.add_argument("--save_path", type=str, default="./act_diff_12-20_gen_2.pt")
    parser.add_argument(
        "--options",
        nargs="+",
        default=[],
...
for idx, row in tqdm(df.iterrows(), total=num_data_points):
    image_path = os.path.join("../datasets/XRD_data/one_correct/loop_scattering", row['image_name'])
    ...
    correct_option = 'Loop scattering'
    wrong_option = 'Background ring'
    ...
    with torch.no_grad(), torch.cuda.amp.autocast():
        ...
        answer_pos_correct = context_embs_correct.shape[1] - 3  # 3rd last token
        ...
        third_last_token_id = last_tensor[0, -3].item()
        third_last_token_text = tokenizer.decode([third_last_token_id])
        tqdm.write(f"EXTRACTING ACTIVATION FROM: Position {answer_pos_correct}, Token ID {third_last_token_id}, Token text '{third_last_token_text}'")
