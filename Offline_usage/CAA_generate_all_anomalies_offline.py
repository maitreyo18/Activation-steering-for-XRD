...
# --- VRAM logging helper (GB) ---
def log_vram_gb(tag="", device=0):
    """Log current/reserved/peak VRAM in GB without altering program behavior."""
    try:
        alloc = torch.cuda.memory_allocated(device) / (1024**3)
        reserved = torch.cuda.memory_reserved(device) / (1024**3)
        peak = torch.cuda.max_memory_allocated(device) / (1024**3)
        tqdm.write(f"[VRAM {tag}] allocated={alloc:.3f} GB | reserved={reserved:.3f} GB | peak={peak:.3f} GB")
    except Exception:
        pass
...
def main():
    ...
    log_vram_gb("after model load", args.gpu_id)
    
    model.eval()
    log_vram_gb("after model.eval()", args.gpu_id)

    tokenizer = AutoTokenizer.from_pretrained(model_config.llama_model)
    log_vram_gb("after tokenizer load", args.gpu_id)
    ...
    log_vram_gb("after vision processor init", args.gpu_id)
    ...
    for idx, row in tqdm(df.iterrows(), total=num_data_points):
        # Per-iteration VRAM snapshot
        log_vram_gb(f"start iter {idx}", args.gpu_id)
        ...
        log_vram_gb(f"after generation {row['image_name']}", args.gpu_id)
        ...
        log_vram_gb(f"after correct anomaly '{args.correct_anomaly}' for {row['image_name']}", args.gpu_id)
        ...
        for wrong_anomaly in args.wrong_anomalies:
            ...
            log_vram_gb(f"after wrong anomaly '{wrong_anomaly}' for {row['image_name']}", args.gpu_id)
        ...
        # End-of-iteration VRAM snapshot
        log_vram_gb(f"end iter {idx}", args.gpu_id)
