...
def parse_args():
    ...
    parser.add_argument("--layers", type=int, nargs="+", default=[20, 22], help="Layer(s) to extract activations from")
    parser.add_argument("--csv_path", type=str, default="train.csv", help="CSV with columns: image_name,prompt,correct_answer")
    parser.add_argument("--save_path", type=str, default="./activation_xrd_20-22.pt")
    parser.add_argument(
        "--options",
        nargs="+",
        default=[],
...
if results_for_csv:
    pd.DataFrame(results_for_csv).to_csv("test_model_responses_20-22_2.csv", index=False)
    print("Wrote model responses to test_model_responses_20-22_2.csv")