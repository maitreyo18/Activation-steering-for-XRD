...
def parse_args():
    ...
    parser.add_argument("--layers", type=int, nargs="+", default=[14, 16, 18], help="Layer(s) to extract activations from")
    parser.add_argument("--csv_path", type=str, default="train.csv", help="CSV with columns: image_name,prompt,correct_answer")
    parser.add_argument("--save_path", type=str, default="./activation_xrd_14-16-18.pt")
    parser.add_argument(
        "--options",
        nargs="+",
        default=[],
...
df = pd.read_csv(args.csv_path).head(400)
...
if results_for_csv:
    ...
    csv_save_path = args.save_path.replace('.pt', '_results.csv')
