...
def parse_args():
    ...
    parser.add_argument("--save_path", type=str, default="./calibration_activations.pt", help="Path to save the activations.")
    parser.add_argument(
        "--options",
        nargs="+",
        default=[],
...
def main():
    ...
    df = df.sample(n=25, random_state=58).reset_index(drop=True)
    reference_acts = {}
    ...
    for idx, row in tqdm(df.iterrows(), total=num_data_points):
        ...
        # Register hooks for layers 20 and 40
        hooks = []
        for layer in [20, 40]:
