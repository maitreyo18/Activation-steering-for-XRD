...
class Prompt:
    ...
    def generate_img_embedding(self, img_prompts):
        ...
        for items in img_prompts:
            ...
            for img in items:
                ...
                try:
                    result = self.model.encode_img(img)
                    feats, _ = result
    ...
    def get_image_token_indices(self):
        """
        Returns the (start_idx, end_idx) of the image tokens in the context embedding sequence for the first image.
        Assumes a single <ImageHere> in the prompt and a single image in img_prompts.
        """
        # Find the length of the first text segment (before <ImageHere>)
        if self.text_embs and self.img_embs:
            text_embs = self.text_embs[0]
            img_embs = self.img_embs[0]
            # text_embs and img_embs are lists of tensors (segments)
            # The first image is after the first text segment
            text_before_len = text_embs[0].shape[1] if len(text_embs) > 0 else 0
            num_image_tokens = img_embs[0].shape[1] if len(img_embs) > 0 else 0
            start_idx = text_before_len
