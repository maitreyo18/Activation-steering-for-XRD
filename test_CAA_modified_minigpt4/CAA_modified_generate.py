...
def parse_args():
    parser = argparse.ArgumentParser(description="Simple CAA Generation with Continuous Steering")
...
def generate_with_steering(prompt, model, tokenizer, steering_vector, alpha, steer_layer, max_new_tokens=150, stop_ids=None):
    """
    ...
    try:
        ...
        print(f"Steering will be applied to last token of prompt and first 2 generated tokens only")
...
def main():
    args = parse_args()
    print("=== CAA Generation with Continuous Steering (Batch Mode) ===")
    ...
    try:
        df = pd.read_csv(args.test_csv).head(10) # head(5) to test
    ...
    filename = f"./Combined_CAA_generation_results_layer_{args.steer_layer}_alpha_{args.alpha}_artifact.csv"
